      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900">{document.ref}</h2>
              <p className="text-sm text-gray-600"><strong>{document.piece}</strong></p>
            </div>
          </div>
          <div style={{backgroundColor: document?.status?.color}} className={`flex items-center space-x-2 px-3 py-1 rounded-full border text-white`}>
      ...
      <div className="py-4">
        <div className="grid  grid-cols-2 md:grid-cols-3 gap-4 mb-6 px-6 ">
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-900 flex gap-2 font-black"><User className="w-5 h-5 text-gray-600" />{document.client_id}</p>
          </div>
          <div className="space-y-1">
            <p className="text-sm font-medium text-gray-500">Expedition</p>
            <p className="text-sm text-gray-900 flex gap-2 font-black"><Truck className="w-5 h-5 text-gray-600" /> {getExped(document.expedition)}</p>
          </div>
        </div>
            ...
            {document.palettes.map((palette) => (
              ...
              >
                ...
                <div className="space-y-1 text-sm text-gray-600">
                  <p>Créé par: <span className='font-semibold'>{palette?.user?.full_name}</span></p>
                  <p className="text-xs">Préparé le:  <span className='font-semibold'>
                    {formatDate(palette.created_at)} </span></p>
                </div>
              </div>
            ))}
          </div>
