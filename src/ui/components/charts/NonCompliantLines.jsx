...
export default function NonCompliantLines() {
  const [dataRow, setDataRow] = useState([]);

  const getData = async () => {
    try {
      const response = await api.get("non-compliant-lines");
  ...
  const data = useMemo(
    () =>
      dataRow.map((d) => ({
        ...
        rate:
          d.total_livraisons > 0
            ? Math.round((d.count_none_compliant_lines / d.total_livraisons) * 100)
            : 0,
      })),
    [dataRow]
  );
        ...
        return `
          <div style="background:#fff;padding:12px 16px;font-size:13px;box-shadow:0 2px 8px rgba(0,0,0,0.08)">
            ...
            <p style="color:#9ca3af;font-size:11px;margin:4px 0 0">
              ${d.count_none_compliant_lines} non conforme${d.count_none_compliant_lines !== 1 ? "s" : ""} / ${d.total_livraisons === 0 ? "—" : d.total_livraisons} livraison${d.total_livraisons !== 1 ? "s" : ""}
            </p>
          </div>`;
      },
    },
    ...
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] mt-4 p-4">
      ...
      <div className="mb-6">
        <h2 className="text-base font-semibold text-gray-900">
          Taux de non conformité fournisseurs
        </h2>
        <p className="text-sm text-gray-400 mt-0.5">
          (non conformes / total livraisons) × 100 — juil. 2025 à juin 2026
        </p>
      </div>
