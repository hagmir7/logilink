  ...
  const navItems = [
    {
      ...
      label: 'Préparation',
      path: '/',
      icon: <ShoppingBag className={window.electron ? 'h-8 w-8 mb-1' : 'h-4 w-4 mb-1'} />,
      color: 'blue',
    },
    {
      key: 'sortie',
    ...
    },
    {
      key: 'transfer',
      label: 'Plus',
      icon: <Plus className={window.electron ? 'h-8 w-8 mb-1' : 'h-4 w-4 mb-1'} />,
      color: 'blue',
      onClick: showModal,
    },
  ];
          ...
          return (
            <button
              ...
              className={`
                ...
                ${!isDisabled && `hover:text-${item.color}-600 hover:bg-${item.color}-50 active:scale-95 transform`}
              `}
            >
              <div className="relative">
                <div
                  className={`transition-transform duration-200 ${
                    !isDisabled && (isActive ? 'scale-110' : 'group-hover:scale-110 group-active:scale-90')
                  }`}
                >
                  {item.icon}
                </div>
              ...
              <span
                className={`${window.electron ? 'text-xl' : 'text-ms'} font-medium transition-colors duration-200 ${
                  !isDisabled && isActive ? `text-${item.color}-600` : ''
                }`}
              >
                {item.label}
              </span>
        ...
        <div className="flex flex-col gap-6 mt-6">
          {roles(['admin', 'magasinier', 'preparation', 'preparation_cuisine', 'preparation_trailer', 'controleur']) && (
            <Button
              ...
              style={window.electron ? { fontSize: '1.5rem', height: 60 } : {}}
              onClick={() => {
                navigate('/transfer-order');
                setIsModalOpen(false);
              }}
            >
              <Package className={window.electron ? 'w-6 h-6' : 'w-4 h-4'} />
          ...
          {roles(['admin', 'magasinier', 'preparation', 'preparation_cuisine', 'preparation_trailer', 'controleur']) && (
            ...
            >
              <ArrowRightLeft className={window.electron ? 'w-6 h-6' : 'w-4 h-4'} />
              Transfert de Stock
            </Button>
          )}
          ...
          {roles(['magasinier', 'admin']) && (
            ...
            >
              <PlaneLanding className={window.electron ? 'w-6 h-6' : 'w-4 h-4'} />
              Reception
            </Button>
          )}

          {roles(['magasinier', 'admin', 'controleur', 'preparation_cuisine']) && (
            <Button
              color="gold"
              variant="solid"
              size="large"
            ...
            >
              <IterationCw className={window.electron ? 'w-6 h-6' : 'w-4 h-4'} />
              Movement de Retour
            </Button>
          )}
          ...
          >
            <LocationEdit className={window.electron ? 'w-6 h-6' : 'w-4 h-4'} />
